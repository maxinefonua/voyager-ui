...
function zoomToLocation(isStart,targetElemId,reviewElem) {
    ...
    if (stringofBoundsArray) {
        ...
        if (isStart) mapFit(airportStartMarkers,bounds);
        else mapFit(airportEndMarkers,bounds);
    } else recenterMap();
...
function addLocationToMap(isStart,longitude,latitude,bounds,stringOfCodesArray) {
    ...
    if (isStart) {
        ...
        if (targetMarkers.length > 0) mapFit(targetMarkers,bounds);
...
function mapFit(targetMarkersArray,bounds) {
    for (const marker of targetMarkersArray) {
        const lngLat = marker.getLngLat();
        bounds[0] = bounds[0] < lngLat.lng ? bounds[0] : lngLat.lng;
        bounds[1] = bounds[1] < lngLat.lat ? bounds[1] : lngLat.lat;
        bounds[2] = bounds[2] > lngLat.lng ? bounds[2] : lngLat.lng;
        bounds[3] = bounds[3] > lngLat.lat ? bounds[3] : lngLat.lat;
    }
